...
function showRoundRobinChart(outputDiv) {
    const roundRobinInput = new Input();
    ...
    const roundRobinChartData = [[], [], [], [], []];
    const timeQuantumArray = [];

    for (let timeQuantum = 1; timeQuantum <= maxTimeQuantum; timeQuantum++) {
        timeQuantumArray.push(timeQuantum);
        const rrInput = new Input();
        setInput(rrInput);
        setAlgorithmNameType(rrInput, 'rr');
        rrInput.timeQuantum = timeQuantum;
        const rrUtility = new Utility();
        setUtility(rrInput, rrUtility);
        const rrOutput = new Output();
        CPUScheduler(rrInput, rrUtility, rrOutput);
        setOutput(rrInput, rrOutput);
        for (let i = 0; i < 4; i++) {
            roundRobinChartData[i].push(rrOutput.averageTimes[i]);
        }
        roundRobinChartData[4].push(rrOutput.contextSwitches);
    }

    const roundRobinChartCanvas = document.createElement('canvas');
    roundRobinChartCanvas.id = "round-robin-chart";

    const roundRobinChartDiv = document.createElement('div');
    ...
    new Chart(roundRobinChartCanvas, {
        type: 'line',
        data: {
        ...
        options: {
            ...
            title: {
                ...
                text: [
                    ...
                    'Completion, Turn Around, Waiting, Response Time & Context Switches',
                    'Lower is Better'
                ],
                fontSize: 18,
...
function showAlgorithmChart(outputDiv) {
    const algorithmArray = ["fcfs", "sjf", "srtf", "ljf", "lrtf", "rr", "hrrn", "pnp", "pp"];
    const algorithmNameArray = ["FCFS", "SJF", "SRTF", "LJF", "LRTF", "RR", "HRRN", "PNP", "PP"];
    const algorithmChartData = [[], [], [], []];

    algorithmArray.forEach(currentAlgorithm => {
        const chartInput = new Input();
        const chartUtility = new Utility();
        const chartOutput = new Output();
        setInput(chartInput);
    ...
    const algorithmChartCanvas = document.createElement('canvas');
    algorithmChartCanvas.id = "algorithm-chart";

    const algorithmChartDiv = document.createElement('div');
    ...
    new Chart(algorithmChartCanvas, {
        type: 'bar',
        data: {
        ...
        options: {
            ...
            title: {
                display: true,
                text: ['Algorithm Comparison', 'Completion, Turn Around, Waiting and Response Time', 'The Lower, The Better'],
                fontSize: 18,
                fontColor: isDark ? '#fff' : '#000',
                padding: 20
            },
